...
class ParametrosCantoneiraParafuso(ParametrosLigacaoBase):
    ...
    def atualizar_opcoes_parafusos(self):
        ...
        try:
            h_w = perfil.h_w  # altura útil do perfil
            n_p_min = 1
            n_p_max = int(math.floor((h_w - 2 * 30) / 67.5))
            self.combo_qtd_parafusos.clear()
            self.combo_qtd_parafusos.addItem(str(1))
            for n in range(n_p_min, max(n_p_min, n_p_max) + 1):
                if n!=1:
                    self.combo_qtd_parafusos.addItem(str(n))
        except Exception as e:
            self.combo_qtd_parafusos.clear()
            self.combo_qtd_parafusos.addItem("1")  # fallback mínimo
