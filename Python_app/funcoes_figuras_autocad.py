...
# Função para limpar completamente o desenho do AutoCAD
def limpar_desenho(acad, intervalo=1):
    """
    Limpa o desenho atual do AutoCAD, repetindo até que esteja completamente vazio.

    Args:
        acad: Instância do Autocad.
        intervalo: Intervalo de tempo em segundos entre tentativas.
    """
    while True:
        objetos_restantes = False
        for obj in acad.iter_objects():
            try:
                obj.Delete()
            except:
                objetos_restantes = True
                continue
        ...
        if not objetos_restantes:
            print("Desenho completamente limpo!")
            break
